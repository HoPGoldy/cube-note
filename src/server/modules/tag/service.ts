...
export const createTagService = (props: Props) => {
  ...
  const getGroupList = async (userId: number) => {
    const data = await db.tagGroup().select('id', 'title').where('createUserId', userId);
    return { code: 200, data };
  };
  ...
  const removeGroup = async (id: number, method: string, userId: number) => {
    ...
    // 删掉下属标签
    if (method === 'force') {
      await db.tag().delete().where({ groupId: id, createUserId: userId });
    }
    // 把该分组下的标签移动到默认分组
    else {
      await db.tag().update('groupId', '').where({ groupId: id, createUserId: userId });
    }
  ...
  const batchSetColor = async (ids: number[], color: string, userId: number) => {
    await db.tag().update('color', color).whereIn('id', ids).andWhere('createUserId', userId);
    return { code: 200 };
  };

  const batchSetGroup = async (ids: number[], groupId: number, userId: number) => {
    await db.tag().update('groupId', groupId).whereIn('id', ids).andWhere('createUserId', userId);
    return { code: 200 };
  };

  const batchRemoveTag = async (ids: number[], userId: number) => {
    await db.tag().delete().whereIn('id', ids).andWhere('createUserId', userId);
    return { code: 200 };
  };
