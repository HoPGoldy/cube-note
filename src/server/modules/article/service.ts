...
export const createArticleService = (props: Props) => {
  ...
  const addArticle = async (title: string, content: string, userId: number, parentId?: number) => {
    ...
    const newArticle: Omit<ArticleStorage, 'id'> = {
      ...
      updateTime: Date.now(),
      parentPath:
        parentArticle && parentId ? appendIdToPath(parentArticle.parentPath, parentId) : '',
      tagIds: '',
    };
  ...
  const removeArticle = async (id: number, force: boolean) => {
    ...
    await db
      .articleRelation()
      .delete()
      .whereIn('fromArticleId', deleteIds)
      .orWhereIn('toArticleId', deleteIds);
    // 删除文章收藏
  ...
  const updateArticle = async (detail: UpdateArticleReqData) => {
    ...
    if (parentArticleId) {
      ...
      await db
        ...
        .update(
          'parentPath',
          db.knex.raw('REPLACE(parentPath, ?, ?)', [oldArticle.parentPath, newArticle.parentPath]),
        )
  ...
  const getArticleList = async (reqData: SearchArticleReqData, userId: number) => {
    ...
    if (keyword) {
      query.whereLike('title', `%${keyword}%`).orWhereLike('content', `%${keyword}%`);
    }
  ...
  const getArticleContent = async (id: number, userId: number) => {
    const article = await db
      ...
      .leftJoin(
        db.knex.raw(
          `${TABLE_NAME.FAVORITE} ON articles.id = favorites.articleId AND favorites.userId = ?`,
          [userId],
        ),
      )
  ...
  const getChildren = async (id: number) => {
    ...
    const query = db
      .article()
      .select('id', 'title', 'parentPath', 'color')
      .whereLike('parentPath', `%#${id}#`);
    // 如果有父级文章，就把父级文章也查出来
  ...
  const getChildrenDetailList = async (id: number) => {
    ...
    const rawData = await db
      .article()
      .select('id', 'title', 'content', 'color', 'tagIds')
      .whereLike('parentPath', `%#${id}#`);
  ...
  const setArticleRelate = async (data: SetArticleRelatedReqData, userId: number) => {
    ...
    if (link) {
      await db.articleRelation().insert({ fromArticleId, toArticleId, userId });
    } else {
      await db.articleRelation().delete().where({ fromArticleId, toArticleId, userId });
    }
  ...
  const arrayToTree = (
    rootId: number,
    data: Pick<ArticleStorage, 'id' | 'title' | 'color' | 'parentPath'>[],
  ) => {
