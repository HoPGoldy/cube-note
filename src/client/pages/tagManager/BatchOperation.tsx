...
export const useBatchOperation = (props: Props) => {
  ...
  const renderBatchBtn = () => {
    ...
    const moveGroupItems = props.tagGroups.map((item) => {
      return {
        key: item.id,
        label: <div onClick={() => onSaveGroup(item.id)}>{item.title}</div>,
      };
    });
    ...
    return (
      <>
        ...
        <Dropdown menu={{ items: moveGroupItems }} placement='bottom'>
          <Button icon={<SwapOutlined />}>批量移动分组</Button>
        </Dropdown>
        <Button onClick={() => setShowColorPicker(true)} icon={<BgColorsOutlined />}>
          批量设置颜色
        </Button>
        ...
        <Row gutter={[8, 8]} ref={transitionRef}>
          {showBatchModal && (
            <>
              ...
              <Col span={8}>
                <Button onClick={onSaveDelete} danger block size='large' icon={<DeleteOutlined />}>
                  删除
                </Button>
              </Col>
              <Col span={8}>
