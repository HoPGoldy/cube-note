...
export const useTagConfig = (props: Props) => {
  ...
  const renderTagDetail = () => {
    return (
      <>
        <Modal open={!!currentTag} closable={false} onCancel={onClose} footer={null} width='24rem'>
          <Row gutter={[16, 16]}>
            ...
            <Col span={12}>
              <Button key='delete' danger block onClick={onDeleteTag}>
                删除标签
              </Button>
            </Col>
            <Col span={12}>
              <Button key='save' type='primary' block onClick={saveChange} loading={isSavingTag}>
                保存
              </Button>
            </Col>
          </Row>
