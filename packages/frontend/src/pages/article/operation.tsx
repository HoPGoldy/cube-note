...
import { useFavoriteArticle } from "@/services/article";
...
import { getFileUrl } from "@/components/file-uploader-plugin";
...
export const useOperation = (props: Props) => {
              ...
              {!isRootArticle && (
                <Col span={8}>
                  <Button
                    ...
                    danger
                    onClick={() =>
                      deleteArticle.showDeleteDialog({
                        title,
                        id: currentArticleId,
                      })
                    }
        ...
        <Space className="text-xl text-gray-500 flex-shrink-0 ml-4">
          ...
          {!isRootArticle && (
            <Button
              icon={<DeleteOutlined />}
              onClick={() =>
                deleteArticle.showDeleteDialog({ title, id: currentArticleId })
              }
            >
              删除
            </Button>
  ...
  const onFileSelect: ChangeEventHandler<HTMLInputElement> = async (event) => {
    ...
    if (!resp.success || !resp.data) {
      messageError(resp.message || "上传失败");
      return;
    }
    ...
    const cm: CodeMirror.Editor = (
      document.querySelector(".CodeMirror.CodeMirror-wrap") as any
    )?.CodeMirror;
    if (!cm) {
