...
import { useSetGroupColor } from "./set-group-color";
import { useTagConfig } from "./tag-config";
import { useBatchOperation } from "./batch-operation";
import { useAllTagGroup, useGroupedTag } from "./tag-hooks";
...
const TagManager: FC = () => {
  ...
  // 新增分组
  const { mutateAsync: addTagGroup, isPending: isAddingGroup } =
    useAddTagGroup();
  // 标题输入框引用
  ...
  // 功能 - 设置分组内标签颜色
  const { renderColorPicker, onClickSetGroupColor } = useSetGroupColor({
    groupedTagDict,
  });
  // 功能 - 标签详情管理
        ...
        <Card
          ...
          extra={
            <Space style={{ margin: "0.8rem 0rem" }}>
              {!isBatch && (
                <Button onClick={() => onClickSetGroupColor(item)}>
                  调整颜色
                </Button>
              )}
  ...
  return (
    <>
      ...
      <PageAction>
        ...
        />
        <ActionButton onClick={onAddGroup} loading={isAddingGroup}>
          新增分组
        </ActionButton>
      </PageAction>
