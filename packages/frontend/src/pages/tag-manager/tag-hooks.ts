...
export const useGroupedTag = (tagList?: TagListItem[]) => {
  // 分组后的标签列表
  const [groupedTagDict, setGroupedTagDict] = useState<
    Record<string | number, TagListItem[]>
  >({});

  useEffect(() => {
    if (!tagList) return;
    const groupedTagList = groupBy(tagList, (item) =>
      item.groupId ? item.groupId : DEFAULT_TAG_GROUP,
    );
...
export const useTagDict = (tagList?: TagListItem[]) => {
  return useMemo(() => {
    const data =
      tagList?.map((item) => [item.id, item] as [number, TagListItem]) || [];
