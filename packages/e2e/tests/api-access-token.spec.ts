...
test.describe("Access Token API", () => {
  ...
  test("GET /api/access-tokens 列表包含已创建的令牌", async ({
    ...
    const found = body.data.find(
      (token: { id: string }) => token.id === createdTokenId,
    );
  ...
  test("缺少 article:write scope 时无法调用写接口", async ({
    request,
    jwt,
  }) => {
    ...
    const readResp = await request.post(`${BASE}/article/getTree`, {
      data: {},
      headers: authHeader(token),
    });
    expect(readResp.status()).toBe(200);

    const writeResp = await request.post(`${BASE}/article/add`, {
      data: {
        title: "scope read only should not write",
      },
      headers: authHeader(token),
    });
  ...
  test("缺少 tag:write scope 时无法修改标签", async ({ request, jwt }) => {
    const createResp = await request.post(`${BASE}/access-tokens`, {
      data: {
        name: "api-e2e-tag-read-token",
        scopes: ["tag:read"],
      },
      headers: authHeader(jwt),
    });
    ...
    const readResp = await request.post(`${BASE}/tag/list`, {
      data: {},
      headers: authHeader(token),
    });
    expect(readResp.status()).toBe(200);

    const writeResp = await request.post(`${BASE}/tag/add`, {
      data: { title: "tag-write-forbidden" },
      headers: authHeader(token),
    });
    expect(writeResp.status()).toBe(403);
  ...
  test("Access Token 不能管理访问令牌", async ({ request }) => {
    const resp = await request.get(`${BASE}/access-tokens`, {
      headers: authHeader(createdTokenPlain),
    });
